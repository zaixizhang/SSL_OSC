...
def main():
    ...
    parser.add_argument('--dataset', type=str, default = 'organic', help='root directory of dataset for pretraining')
    parser.add_argument('--input_model_file', type=str, default='',
                        help='filename to read the model (if there is any)')
    parser.add_argument('--output_model_file', type=str, default = './model_gin/mask', help='filename to output the model')
    ...
    dataset = MoleculeDataset("dataset/organic_train", dataset=args.dataset, transform = MaskAtom(num_atom_type = 119, num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge))
    train_dataset, _, _ = random_split(dataset, null_value=0, frac_train=1.0, frac_valid=0.0,
                                       frac_test=0.0, seed=args.seed)
    loader = DataLoaderMasking(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers = args.num_workers)
