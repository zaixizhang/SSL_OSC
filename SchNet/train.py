...
def train(modelname="sch", epochs=80, device=th.device("cuda:3")):
    logger = Logger(paras, paras['logger_flag'], filename=paras['logger_name'])
    print(device)
    print("starting")
    alchemy_dataset = TencentAlchemyDataset(mode='train', dataset=paras['dataset'])
    alchemy_loader = DataLoader(dataset=alchemy_dataset,
                                batch_size=paras['batch_size'],
                                collate_fn=batcher(),
                                shuffle=True)

    test_dataset = TencentAlchemyDataset(mode='train', dataset=paras['test_dataset'])
    ...
    if modelname == 'sch':
        model = SchNetModel(cutoff=paras['cutoff'], width=paras['width'], n_conv=paras['n_layers'], mmd_flag=False)
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(alchemy_loader):
            ...
            if paras['ref_flag']:
                y_ref = batched_ref(X.ndata['node_type'].tolist(), X.batch_num_nodes, paras['hartree_flag'], paras['property_id'])
